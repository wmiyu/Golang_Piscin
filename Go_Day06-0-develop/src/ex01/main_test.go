...
func TestStaticFileServer(t *testing.T) {
	...
	if resp.StatusCode != http.StatusOK {
		t.Errorf("Status shoud be 200. got %d", resp.StatusCode)
	}
	connectRype := resp.Header.Get("Content-Type")
	expectedConnectType := "text/html; charset=utf-8"

	if expectedConnectType != connectRype {
		t.Errorf("Wroing content type, expected %s, got %s", expectedConnectType, connectRype)
	}
}